...
public class AIBehaivor2 {
	...
	public static void seek(GameObject go, AIdata2 aiAdata){
		// calculate the steering velocity
		Vector3 pos_d = aiAdata.target.transform.position - go.transform.position;
		...
		if (d < Time.deltaTime * aiAdata.maxSpeed * 2.0f) {
			go.transform.position = aiAdata.target.transform.position;
			return;
		}
